...
<form class="cart grouped_form"
    ...
    <table cellspacing="0" class="woocommerce-grouped-product-list group_table b-product_single_action clearfix">
        <tbody>
        <?php
                    ...
                    case 'quantity':
                        ...
                        if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
                            ...
                            <div class="quantity clearfix">
                                ...
                                <input type="text" step="1" min="<?= $min_value ?>" max="<?= $max_value ?>"
                                       name="<?= $input_name ?>"
                                       value="<?= $input_value ?>"
                                       title="Qty"
                                       class="input-text qty text prod-quantity" size="4" pattern="[0-9]*"
                                       inputmode="numeric">
    ...
    <?php if ( $quantites_required ) : ?>
        ...
        <div class="b-product_single_action clearfix">
            <div class="quantity clearfix">
                <button type="submit"
                        class="single_add_to_cart_button button button-large m-0"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
            </div>
        </div>
